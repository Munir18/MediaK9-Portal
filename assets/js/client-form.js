...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (codeInput) {
    ...
    codeInput.addEventListener('input', () => {
      // Strip non-digits
      codeInput.value = codeInput.value.replace(/[^0-9]/g, '').slice(0, 3);
      ...
      if (!/^\d{3}$/.test(val)) {
        setCodeStatus('checking', 'Enter exactly 3 digits...');
        return;
      }
      ...
      codeTimer = setTimeout(async () => {
        try {
          // Send both formats; backend normalizes and checks both
          ...
          if (res.success) {
            ...
            if (codeBox) codeBox.style.borderColor = 'rgba(74,222,128,.4)';
          } else {
            codeValid = false;
  ...
  const contactPhone = document.getElementById('mk9-contact-phone');
  if (contactPhone) {
    contactPhone.addEventListener('input', () => {
      let val = contactPhone.value.replace(/\D/g, '');
      if (val.length > 11) val = val.slice(0, 11);
      if (val.length > 4) {
        contactPhone.value = val.slice(0, 4) + '-' + val.slice(4);
      } else {
        contactPhone.value = val;
      }
    });
  }
  ...
  const validateClientField = (input) => {
    ...
    } else if (input.id === 'mk9-contact-phone' && input.value && !/^\d{4}-\d{7}$/.test(input.value)) {
      valid = false; msg = 'Phone must be exactly 11 digits (e.g. 0318-6072309).';
    }
